...
const Esim: React.FC = () => {
  const { data: plans = [] } = usePlans({ simType: "esim" });

  const esimPlans = plans.filter((plan: any) =>
    plan.simTypes?.includes("esim")
  );

  const formattedPlans = esimPlans.map((plan: any) => ({
    id: plan._id,
    name: `${plan.operatorId?.name} ${plan.planTypeId?.name} Plan`,
  ...
  const serviceMap: any = {
    "new-connection": { path: "/simstore",    name: "New Connection", icon: <FaSimCard /> },
    "port-number":    { path: "/portnumber",  name: "Port Number",    icon: <MdSwapHoriz /> },
    "fancy-number":   { path: "/fancynumber", name: "Fancy Number",   icon: <RiVipCrownFill /> },
    "business-sim":   { path: "/businesssim", name: "Business SIM",   icon: <FaBusinessTime /> },
    "e-sim":          { path: "/esim",        name: "eSIM",           icon: <FaPhoneAlt /> },
  };

  const orderedSlugs = ["new-connection", "port-number", "fancy-number", "business-sim", "e-sim"];
        ...
        >
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <div style={{
              ...
              <FaPhoneAlt style={{ fontSize: 11 }} /> eSIM Plans
            </div>
            ...
            <p style={{ margin: "8px 0 0", opacity: 0.85, fontSize: 14 }}>
              Instant digital SIM activation ⚡
            </p>
          </div>
        ...
        >
          ...
          {formattedPlans.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: "#94A3B8" }}>
              ...
              <p style={{ fontWeight: 600, fontSize: 14 }}>No eSIM plans available right now</p>
            </div>
          ) : (
              ...
              {formattedPlans.map((plan, i) => (
                <motion.div
                  ...
                  style={{
                    ...
                    border: `1.5px solid ${plan.accent}22`,
                  ...
                  onMouseLeave={(e) => {
                    ...
                    (e.currentTarget as HTMLElement).style.borderColor = `${plan.accent}22`;
                  }}
                >
                  {/* Tinted header */}
                  <div style={{
                  ...
                  {/* CTA — navigates to PlanDetails, which owns the full checkout flow */}
                  <div style={{ padding: "0 18px" }}>
                    <button
                     onClick={() => navigate(`/plan-details/${plan.id}`, { state: { orderType: "ESIM" } })}
                      style={{
